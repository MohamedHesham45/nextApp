...
const V2FiveProductsPerCategory = () => {
    ...
    const categoryId =
        filteredProducts.length > 0
            ? filteredProducts[0].categoryId || filteredProducts[0].categoryID || filteredProducts[0].category
            : selectedCategory;
                    ...
                    <div className="flex space-x-2 w-max pb-2">
                        {categories.map((c, i) => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-full text-sm font-medium whitespace-nowrap transition-colors duration-300 ${selectedCategory === c.category
            ...
            {viewMode === "grid" ? (
                <>
                    <div className="grid grid-cols-3 gap-[1px]">
                        {filteredProducts.map((product) => (
                            <V2ProductCardSimple key={product._id} product={product} viewMode="grid" />
                        ))}
                    </div>
                    {/* View All Button */}
                    {categoryId && (
                 ...
                 hover:bg-amazon-orange-dark hover:shadow-lg hover:scale-105"
                            >
                                <span>
                                    عرض كل منتجات ال
                                    {categories.find((c) => c.category === selectedCategory)?.category || "المنتجات"}
                                </span>
                                <svg
                ...
                <>
                    <div className="flex flex-col gap-1">
                        {filteredProducts.map((product) => (
                            <V2ProductCardSimple key={product._id} product={product} viewMode="list" />
                        ))}
                    </div>
                    {/* View All Button */}
                    {categoryId && (
