...
export default function DeliveryTypes() {
    ...
    const getShippingPrice = () => {
        ...
        const governorate = governorates.find(gov => gov._id === selectedGovernorate);
        if (!governorate?.shippingPrices) return null;
        const shippingPrice = governorate.shippingPrices.find(price => price.shippingTypeId === selectedShippingType);
    ...
    return (
        <div className="container mx-auto px-4 py-8 direction-rtl text-amazon-dark-gray bg-gray-50">
            ...
            <section className="mb-12">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {shippingTypes.map((type, index) => (
                        <div key={type._id} className="p-4 rounded-lg border shadow-md bg-white border-r-4 border-amazon-yellow">
            ...
            <section className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div>
                    <label className="block mb-2 font-semibold text-amazon-dark-gray">نوع الشحن</label>
                    <select
                ...
                <div>
                    <label className="block mb-2 font-semibold text-amazon-dark-gray">المحافظة</label>
                    <select
                            ...
                            <div className="flex items-center justify-between gap-4">
                                ...
                                <div>
                                    <h4 className="text-lg font-semibold m-0">
                                        {shippingTypes.find(type => type._id === selectedShippingType)?.name}
                                    </h4>
                                    <p className="text-sm">
                                        {governorates.find(gov => gov._id === selectedGovernorate)?.nameAr}
                                    </p>
                                </div>
                            </div>
