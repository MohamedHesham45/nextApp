...
export default function OrdersPage() {
  useScrollCache("v2-admin-orders");
  ...
  const fetchOrders = async () => {
    try {
      setLoading(true);
      const response = await fetch("/v2/api/orders");
      if (!response.ok) {
      ...
      setError("فشل في تحميل الطلبات. يرجى المحاولة مرة أخرى لاحقًا.");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpdateStatus = async (orderId, newStatus) => {
    try {
      const response = await fetch(`/v2/api/orders/${orderId}`, {
        method: "PATCH",
        headers: {
              ...
              <table className="min-w-full bg-white">
                <thead>
                  <tr className="bg-gray-50 text-gray-600 uppercase text-sm leading-normal text-center">
                    ...
                    <th className="py-3 px-6 ">اسم العميل</th>
                    <th className="py-3 px-6 ">السعر الإجمالي</th>
            ...
            >
              <div className="mt-3">
                ...
                <p className="text-sm text-gray-500 mb-1">
                  اسم العميل : {selectedOrder.customerDetails.name}
                </p>
                <p className="text-sm text-gray-500 mb-1">
                  رقم الهاتف : {selectedOrder.customerDetails.phone}
                </p>
                <p className="text-sm text-gray-500 mb-1">
                ...
                <div className="mt-4">
                  ...
                  <div className="space-y-2 text-sm text-gray-500">
                    <p>
                      <span className="font-medium me-1">المحافظة :</span>{" "}
                      {selectedOrder.customerDetails.governorate}
                    </p>
                    <p>
                      <span className="font-medium">تفاصيل العنوان:</span>{" "}
                      {(() => {
                        const addressParts =
                          selectedOrder.customerDetails.centerArea.split("،");
                        return (
                          <>
                    ...
                    <p>
                      <span className="font-medium me-1">الحي :</span>{" "}
                      {selectedOrder.customerDetails.neighborhood}
                    </p>
                    <p>
                      <span className="font-medium me-1">طريقة الاتصال :</span>{" "}
                      {selectedOrder.customerDetails.preferredContactMethod}
                    </p>
                    <p>
                      <span className="font-medium me-1">نوع الدفع :</span>{" "}
                      {selectedOrder.customerDetails.buyType}
                    </p>
                    <p>
                      <span className="font-medium me-1">نوع الشحن :</span>{" "}
                      {selectedOrder.customerDetails.shippingType}
                    </p>
                    {selectedOrder.customerDetails.deliveryInstructions && (
                      <p>
                    ...
                    {selectedOrder.customerDetails.message && (
                      <p>
                        <span className="font-medium me-1">رسائل اضافية :</span>{" "}
                        {selectedOrder.customerDetails.message}
                      </p>
                    )}
                  </div>
                ...
                <p className="text-sm font-medium text-gray-900 mt-4 mb-8">
                  السعر :{selectedOrder.totalPrice.toFixed(2)} ج.م
                </p>
                <p className="text-sm font-medium text-gray-900 mt-4 mb-8">
                  سعر التوصيل:
                  {selectedOrder.shippingCost.toFixed(2)} ج.م
                </p>
                <p className="text-sm font-medium text-gray-900 mt-4 mb-8">
                  السعر الإجمالي:
