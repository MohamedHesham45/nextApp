...
export default function OrderPage({ params }) {
    ...
    const fetchOrder = async () => {
        try {
            ...
            const governorateId = governorates.find(g => g.nameAr === data.customerDetails.governorate)?._id;

            // Fetch neighborhoods for the selected governorate
            if (governorateId) {
                const neighborhoodResponse = await fetch(`/api/governorate/neighborhood/governorate/${governorateId}`);
                const neighborhoodData = await neighborhoodResponse.json();
                setNeighborhoods(neighborhoodData);
            }

            // Find the shipping type ID based on the name
            const shippingTypeId = shippingTypes.find(st => st.name === data.customerDetails.shippingType)?._id;
            // Find the buy type ID based on the name
            const buyTypeId = buyTypes.find(bt => bt.name === data.customerDetails.buyType)?._id;

            // Split the centerArea into its components
            const [areaValue = '', streetValue = '', architectureValue = '', apartmentValue = ''] = (data.customerDetails.centerArea || '').split('،').map(s => s.trim());
            ...
            setFormData({
                ...
                preferredContactMethod: data.customerDetails.preferredContactMethod || 'واتساب',
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const [governoratesRes, shippingTypesRes, buyTypesRes] = await Promise.all([
    ...
    const calculateShippingCost = () => {
        ...
        const governorate = governorates.find(g => g._id === formData.governorate);
            ...
            const response = await fetch(`/api/orders/${params.orderId}`, {
                ...
                body: JSON.stringify({
                    ...
                    customerDetails: {
                        ...
                        governorate: governorates.find(g => g._id === formData.governorate)?.nameAr || order.customerDetails.governorate,
                        neighborhood: formData.neighborhood,
                        centerArea: formData.centerArea,
                        buyType: buyTypes.find(bt => bt._id === formData.buyType)?.name || order.customerDetails.buyType,
                        shippingType: shippingTypes.find(st => st._id === formData.shippingType)?.name || order.customerDetails.shippingType,
    ...
    const handleUpdateItemQuantity = async () => {
        ...
        try {
            ...
            if (selectedItem.category?.minCount && newQuantity < selectedItem.category.minCount) {
    ...
    return (
        <div className="min-h-screen bg-gradient-to-b from-amazon-light-gray to-white">
                                ...
                                <div className="text-center sm:text-right bg-gradient-to-r from-amazon-yellow/10 to-white p-4 rounded-xl border border-amazon-yellow w-full sm:w-auto">
                                    <div className="flex flex-col gap-2">
                                        <p className="text-sm text-amazon-dark-gray">
                                            تكلفة المنتجات: {order.orderItems.reduce((total, item) => total + (item.price * item.quantity), 0)} ج.م
                                        </p>
                                        <p className="text-sm text-amazon-dark-gray">
                                            تكلفة الشحن: {order.shippingCost} ج.م
                                        </p>
                                        <p className="text-xl sm:text-2xl font-bold text-amazon-orange">
                                            الإجمالي: {order.orderItems.reduce((total, item) => total + (item.price * item.quantity), 0) + Number(order.shippingCost)} ج.م
                                        </p>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div className="space-y-4 h-[70vh] overflow-y-auto">
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full p-3 border ${formErrors.name ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                />
                                {formErrors.name && (
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full p-3 border ${formErrors.phone ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                />
                                {formErrors.phone && (
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    className={`w-full p-3 border ${formErrors.email ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                />
                                {formErrors.email && (
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={async (e) => {
                                        ...
                                        if (newGovernorateId) {
                                            try {
                                                const response = await fetch(`/api/governorate/neighborhood/governorate/${newGovernorateId}`);
                                                const data = await response.json();
                                                setNeighborhoods(data);

                                                // Check if current neighborhood exists in new list
                                                const neighborhoodExists = data.some(n => n.nameAr === formData.neighborhood);
                                    ...
                                    className={`w-full p-3 border ${formErrors.governorate ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                >
                                ...
                                {formErrors.governorate && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.governorate}</p>
                                )}
                            </div>
                            <div>
                                ...
                                <select
                                    ...
                                    className={`w-full p-3 border ${formErrors.neighborhood ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                >
                                ...
                                {formErrors.neighborhood && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.neighborhood}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-amazon mb-2 text-right">المركز/المنطقة</label>
                                <input
                                    type="text"
                                    value={area}
                                    onChange={(e) => setArea(e.target.value)}
                                    className={`w-full p-3 border ${formErrors.area ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                />
                                {formErrors.area && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.area}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-amazon mb-2 text-right">الشارع</label>
                                <input
                                    type="text"
                                    value={street}
                                    onChange={(e) => setStreet(e.target.value)}
                                    className={`w-full p-3 border ${formErrors.street ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                />
                                {formErrors.street && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.street}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-amazon mb-2 text-right">رقم العمارة</label>
                                <input
                                    type="text"
                                    value={architecture}
                                    onChange={(e) => setArchitecture(e.target.value)}
                                    className={`w-full p-3 border ${formErrors.architecture ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                />
                                {formErrors.architecture && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.architecture}</p>
                                )}
                            </div>
                            <div>
                                <label className="block text-amazon mb-2 text-right">رقم الشقة</label>
                                <input
                                    type="text"
                                    value={apartment}
                                    onChange={(e) => setApartment(e.target.value)}
                                    className={`w-full p-3 border ${formErrors.apartment ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                />
                                {formErrors.apartment && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.apartment}</p>
                                )}
                            </div>
                            <div>
                                ...
                                <select
                                    ...
                                    onChange={(e) => {
                                        ...
                                        setFormErrors({ ...formErrors, preferredContactMethod: '' });
                                    }}
                                    className={`w-full p-3 border ${formErrors.preferredContactMethod ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                >
                                    <option value="واتساب">واتساب</option>
                                    <option value="اتصال">اتصال</option>
                                </select>
                                {formErrors.preferredContactMethod && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.preferredContactMethod}</p>
                                )}
                            </div>
                            <div>
                                ...
                                <select
                                    ...
                                    className={`w-full p-3 border ${formErrors.shippingType ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                >
                                ...
                                {formErrors.shippingType && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.shippingType}</p>
                                )}
                            </div>
                            <div>
                                ...
                                <select
                                    ...
                                    className={`w-full p-3 border ${formErrors.buyType ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                >
                                ...
                                {formErrors.buyType && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.buyType}</p>
                                )}
                            </div>
                            <div>
                                ...
                                <textarea
                                    ...
                                    className={`w-full p-3 border ${formErrors.deliveryInstructions ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                    rows="2"
                                />
                                {formErrors.deliveryInstructions && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.deliveryInstructions}</p>
                                )}
                            </div>
                            <div>
                                ...
                                <textarea
                                    ...
                                    className={`w-full p-3 border ${formErrors.message ? 'border-red-500' : 'border-amazon-light-gray'} rounded-xl text-right`}
                                    rows="2"
                                />
                                {formErrors.message && (
                                    <p className="text-red-500 text-sm mt-1 text-right">{formErrors.message}</p>
                                )}
                            </div>
                        </div>
            ...
            {isCancelModalOpen && (
                <div className="fixed inset-0 bg-amazon bg-opacity-50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
                    <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-[32rem]">
                        <h2 className="text-xl sm:text-2xl font-bold mb-6 text-right text-amazon">تأكيد إلغاء الطلب</h2>
                        <p className="text-right mb-8 text-amazon-dark-gray">هل أنت متأكد من رغبتك في إلغاء هذا الطلب؟</p>
            ...
            {isModalOpen && modalMode === 'item' && (
                <div className="fixed inset-0 bg-amazon bg-opacity-50 backdrop-blur-sm flex justify-center items-center z-50 p-4">
                    <div className="bg-white p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-[32rem]">
                        <h2 className="text-xl sm:text-2xl font-bold mb-6 text-right text-amazon">تعديل كمية المنتج</h2>
                        <div>
