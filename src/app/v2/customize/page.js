...
export default function CustomizePage() {
  ...
  const fetchCustomFields = async () => {
    try {
      const response = await fetch("/v2/api/customize");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url =
        modalMode === "add"
          ? "/v2/api/customize"
          : `/v2/api/customize/${selectedField._id}`;
      ...
      if (formData.name === "صوره الوجهه" && formData.value instanceof File) {
        ...
        if (!uploadResponse.ok)
          throw new Error("حدث خطأ أثناء رفع الصور حاول مرة أخرى");
  ...
  const confirmDelete = async () => {
    setSubmitLoading(true);
    try {
      const response = await fetch(`/v2/api/customize/${selectedField._id}`, {
        method: "DELETE",
      });
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {customFields.map((field) => (
            ...
            >
              ...
              <p className="overflow-hidden text-ellipsis whitespace-nowrap text-gray-600 text-right">
                {field.value}
              </p>
            </div>
          ))}
            ...
            <div className="bg-white p-6 rounded shadow-lg w-96">
              <>
                <h2 className="text-xl font-semibold mb-4 text-right">
                  {modalMode === "add" ? "إضافة حقل جديد" : "تعديل الحقل"}
                </h2>
