...
export function ProductDetails() {
    ...
    return (
        <div className="container mx-auto px-6 md:px-16 lg:px-32 py-16 direction-rtl bg-gray-50 min-h-screen">
                            ...
                            <div className="flex gap-4 overflow-x-auto pb-2">
                                {images.map((image, index) => (
                                    <button
                                        ...
                                        className={`relative flex-shrink-0 w-24 h-24 rounded-lg overflow-hidden ${selectedImageIndex === index
                            ...
                            {product.discountPercentage > 0 ? (
                                <div className="space-y-3">
                                    <p className="text-gray-500 text-lg">
                                        قبل الخصم: <span className="text-red-500 line-through">{product.price.toFixed(2)} جنيه</span>
                                    </p>
                                    <p className="text-gray-500 text-lg">
                                        بعد الخصم: <span className="text-green-600 font-bold text-3xl">{product.priceAfterDiscount.toFixed(2)} جنيه</span>
                                    </p>
                                </div>
                            ) : (
                                <p className="text-gray-500 text-lg">
                                    السعر: <span className="text-green-600 font-bold text-3xl">{product.price.toFixed(2)} جنيه</span>
                                </p>
                            )}

                            <p className="text-gray-500 text-lg">
                                الكمية المتوفرة: {' '}
                                <span className={`font-bold ${product.quantity > 0 ? 'text-green-600' : 'text-red-500'}`}>
